...
.summaryStatsByCountry <- function(data,countryYear){
...
#   forOwnerU <- forOwner[2]
  ...
  if (sector == "Manufacturing") {
    ...
    if (ageRange == "All firms") {
      ageRange <- c("0-5","6-15","16-30","+30")
    }
    if (expRange == "All firms") {
      expRange <- c("0-25%","26-50%","51-75%","+75%")
    }
    if (ownRange == "All firms") {
      ownRange <- c("0-50%","50-75%","+75%")
    }
    
    data <- data %>%
      group_by(country,idstd) %>%
      mutate(ageVal = as.numeric(thisYear) - b5,
             age = as.character(ifelse(ageVal < 6,"0-5",ifelse(ageVal < 16,"6-15",ifelse(ageVal < 31,"16-30","+30")))),
             size = as.character(ifelse(l1 < 20,"Small firm",ifelse(l1 < 100,"Medium firm","Large firm"))), 
             expVal = d3b + d3c,
             expStatus <- as.character(ifelse(expVal < 26,"0-25%",ifelse(expVal < 51,"26-50%",ifelse(expVal < 75,"51-75%","+76%")))),
             forOwner = as.character(ifelse(b2a < 51,"0-50%",ifelse(b2a < 76,"50-75%","+75%")))) %>% # filter by age, size, etc...
  ...
  if (nrow(data[data$country==countryYear,])>=5){
    ...
    # Remove duplicates, I only need 1 row  
...
# Iterate countries -----------------------------------
.summaryStats <- function(sector,indicatorDesc,allocEff,
                                   ageRange,sizeRange,expRange,ownRange){
